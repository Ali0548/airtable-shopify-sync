  ...
  try {
    ...
    allOrders?.forEach((order) => {
      ...
      const record = records?.find(
        (x) => x?.fields?.["Order Number"] === orderNumber
      );
      if (record) {
        const data = transformedDbOrderToUpdateAirTableOrder(order);
        // console.log("data", data);
        orderToUpsert?.push(data);
      } else {
        const data = transformedDbOrderToCreateAirTableOrder(order);
        console.log("data", data);
        ordersToCreate?.push(data);
      }
    ...
    console.log("ordersToCreate", ordersToCreate?.length);
    console.log("orderToUpsert", orderToUpsert?.length);
    ...
    if (ordersToCreate?.length > 0) {
      console.log(
        `🔄 Creating ${ordersToCreate.length} records in batches of 10...`
      );
      ...
      for (let i = 0; i < createBatches.length; i++) {
        console.log(
          `📦 Processing create batch ${i + 1}/${createBatches.length} (${
            createBatches[i].length
          } records)`
        );
        const batchResult = await createRecord(
          "VIVANTI LONDON ORDER TRACKING",
          createBatches[i]
        );
        if (batchResult?.errors?.length === 0) {
          ...
          for (const bRecord of batchResultData) {
            const order = await OrderModel.findOne({
              legacyResourceId: extractNumberAfterDash(
                bRecord?.fields?.["Order Number"]
              ),
            });
    ...
    if (orderToUpsert?.length > 0) {
      console.log(
        `🔄 Upserting ${orderToUpsert.length} records in batches of 10...`
      );
      ...
      for (let i = 0; i < upsertBatches.length; i++) {
        console.log(
          `📦 Processing upsert batch ${i + 1}/${upsertBatches.length} (${
            upsertBatches[i].length
          } records)`
        );
        const batchResult = await upsertRecord(
          "VIVANTI LONDON ORDER TRACKING",
          upsertBatches[i]
        );
...
const getAllOrdersAndSyncToAirTable = async (
  triggeredBy = "manual",
  metadata = {}
) => {
  let syncJob = null;
  ...
  try {
    ...
    const {
      ...
      data,
      success,
    } = await shopifyWrapper.getAllOrders(parseInt(150));

    if (!success || (shopifyErrors && shopifyErrors.length > 0)) {
      const error = {
        stage: "shopify_fetch",
        message:
          shopifyErrors[0].userMessage || "Failed to fetch orders from Shopify",
    ...
    console.log(
      `✅ Fetched ${syncJob.shopifyOrdersFetched} orders from Shopify`
    );
    ...
    try {
      ...
      console.log(
        `✅ Upserted ${syncJob.shopifyOrdersUpserted} orders to database`
      );
    } catch (upsertError) {
      const error = {
    ...
    try {
      ...
      if (airtableResponse.success) {
        syncJob.airtableRecordsCreated =
          airtableResponse.createResponse?.totalRecords || 0;
        syncJob.airtableRecordsUpdated =
          airtableResponse.upsertResponse?.totalRecords || 0;
        await syncJob.save();

        console.log(
          `✅ AirTable sync completed: ${syncJob.airtableRecordsCreated} created, ${syncJob.airtableRecordsUpdated} updated`
        );
      } else {
        const error = {
          stage: "airtable_sync",
    ...
    return {
      ...
      data: {
        ...
        summary,
        shopifyResult: {
          ordersFetched: syncJob.shopifyOrdersFetched,
          ordersUpserted: syncJob.shopifyOrdersUpserted,
        },
